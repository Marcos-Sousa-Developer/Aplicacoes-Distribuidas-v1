...
if __name__ == '__main__':
    ...
    parser.add_argument('K', type=int, help='Número de bloqueios permitidos em cada recurso')

    argument = parser.parse_args()

    lp = lock_pool(argument.N, argument.K)
    ...
    while True:
        ...
        try:
            ...
            print("Ligação estabelecida -> Host: {} e Port: {}".format(addr,port))
            ...
            if "LOCK-W" in request:
        
                answer = lp.lock('W', int(request[1]) , int(request[-1]), int(request[2]))
                

            elif "LOCK-R" in request:

                answer = lp.lock('R', int(request[1]) , int(request[-1]), int(request[2]))


            elif "UNLOCK-W" in request:

                answer = lp.unlock('W', int(request[1]) , int(request[-1]) )

            elif "UNLOCK-R" in request:

                answer = lp.unlock('R', int(request[1]) , int(request[-1]) )

            elif "STATUS" in request:

                answer = lp.status(int(request[-1]))

            elif "STATS" in request:

                if 'K' in request:
                    answer = lp.stats('K',int(request[-1]))

                else:
                    answer = lp.stats(request[1],None)

            elif "PRINT" in request:

                answer = lp
              
            conn_sock.sendall(str(answer).encode('utf-8'))

            if "PRINT" not in request:
